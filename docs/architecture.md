...
6. **`ContentRootPath` must be pinned to `AppContext.BaseDirectory` explicitly.** The generic host defaults it to the process's current working directory, not the exe's folder. Left at the default, `appsettings.json` is invisible for any launch whose CWD is not that folder — a shortcut, Task Scheduler, or simply running the exe from a repo checkout — and every setting silently falls back to its C# default with no error. This was found live: `ExcludedProcesses` and `ExcludedTitlePatterns` had likely never been active outside a launch from the exe's own directory. Run `--config` to see what actually loaded, including content root, versus what a JSON file claims.
